...
class Repository(pygit2.Repository):
    ...
    # split the ref in branch or tag
    def split_ref(self, path, expects_file=False):
        file = None
        # First extract branch, which can contain slashes
        for ref in sorted(self.branches(), key=lambda x: -len(x)):
            if path.startswith(ref):
                path = path.replace(ref, '')[1:]
                tree = self[self.lookup_reference('refs/heads/%s' % ref).target.hex].tree
                break
        else:
            # OK, maybe a tag?
            for ref in sorted(self.tags(), key=lambda x: -len(x)):
                if path.startswith(ref):
                    path = path.replace(ref, '')[1:]
                    ref = self.lookup_reference('refs/tags/%s' % ref).target.hex
                    if self[ref].type == pygit2.GIT_OBJ_TAG:
                        ref = self[self[ref].target].hex
                    tree = self[ref].tree
                    break
            else:
                # Or a commit
                ...
                try:
                    ...
                    raise IOError("No such commit/ref")
